...
export type CloseFriendItem = {
  ...
  isInCloseFriends?: boolean
}

type CloseFriendsResponse = {
  message?: string
  data?: CloseFriendItem[]
  page?: number
  pageSize?: number
  maxPageSize?: number
  totalPages?: number
...
async function fetchCloseFriendsPage(page: number): Promise<CloseFriendsResponse> {
  const qs = new URLSearchParams({
    page: String(page),
    maxPageSize: String(PAGE_SIZE),
  })

  const res = await apiFetch(`${API_URL}/close_friends?${qs.toString()}`, {
    method: "GET",
    cache: "no-store",
  })
  ...
  return (await res.json().catch(() => ({}))) as CloseFriendsResponse
}
...
export function useCloseFriends() {
  const q = useInfiniteQuery({
    ...
    initialPageParam: 1,
    queryFn: ({ pageParam }) => fetchCloseFriendsPage(Number(pageParam)),
