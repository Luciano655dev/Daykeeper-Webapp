...
async function fetchDailyTasksPage(page: number): Promise<DailyTasksResponse> {
  const qs = new URLSearchParams({
    page: String(page),
    maxPageSize: String(PAGE_SIZE),
  })

  const res = await apiFetch(`${API_URL}/day/dailyTasks?${qs.toString()}`, {
    method: "GET",
    cache: "no-store",
  })
  ...
  return (await res.json().catch(() => ({}))) as DailyTasksResponse
}
...
export function useDailyTasks() {
  const q = useInfiniteQuery({
    queryKey: ["dailyTasks"],
    initialPageParam: 1,
    queryFn: ({ pageParam }) => fetchDailyTasksPage(Number(pageParam)),
